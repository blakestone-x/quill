...
export default function StartScreen({
  ...
  return (
    <div className="absolute inset-0 overflow-y-auto bg-ink-900 no-drag">
      <div className="max-w-[820px] mx-auto px-8 py-8">
        <div className="mb-6 text-center">
          <p className="text-xs text-paper-200/60">
            Start typing a title above, or jump to a recent note below. Press{' '}
            <span className="font-mono text-paper-200">Ctrl+F</span> to search everything.
            {cartographAvailable && (
              <span className="block mt-1 text-amber-400/80">
                Cartograph connected — context flows both ways.
              </span>
            )}
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={onSearch}
          className="w-full mb-6 flex items-center gap-3 px-4 py-3 bg-ink-800 border border-ink-700 hover:border-amber-500/60 rounded-md text-paper-200 hover:text-paper-50 transition-colors text-sm"
        >
          <Search size={14} />
        ...
        {recent.length === 0 && pinned.length === 0 && (
          <div className="text-center text-paper-200/40 text-xs font-mono py-8">
            no other notes — start typing up top
          </div>
        )}
      </div>
    </div>
